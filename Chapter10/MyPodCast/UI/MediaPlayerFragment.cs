...
namespace MyPodCast
{
	public class MediaPlayerFragment : Fragment
	{
		...
		public MediaPlayerFragment()
		{
            ...
            {
				Logger.Debug("QueueFragment.OnConnectedImpl");
			...
			_sessionCallback.OnPlaybackStateChangedImpl = state => 
            {
				...
				Logger.Debug("OnPlaybackStateChangedImpl");
				_playbackState = state;
				OnPlaybackStateChanged(state);
			};
			_sessionCallback.OnQueueChangedImpl = queue => 
            {
				Logger.Debug("OnQueueChangedImpl ");
				if(queue != null) {
					_queueAdapter.Clear();
					_queueAdapter.NotifyDataSetInvalidated();
					_queueAdapter.AddAll(queue.ToArray());
		...
		{
			Logger.Debug("OnPlaybackStateChanged ");
			...
			Logger.Debug(statusBuilder.ToString());
